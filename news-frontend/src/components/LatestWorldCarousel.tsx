...
interface LatestWorldCarouselProps {
  limit?: number;
}

const LatestWorldCarousel: React.FC<LatestWorldCarouselProps> = ({ 
  limit = 10 
}) => {
  ...
  // Fetch latest world news data
  useEffect(() => {
    const fetchLatestWorldNews = async () => {
      try {
        setLoading(true);
        setError(null);
        
        console.log('🌍 Fetching latest world news for carousel with limit:', limit);
        const data = await cachedTrendingApi.getLatestWorldNewsForCarousel(limit);
        
        console.log('📊 Received latest world news for carousel:', data.length, 'articles');
        
        // Validate that we only have world trending news
        const validatedWorldData = data.filter(item => {
          const source = item.source?.toLowerCase();
          const category = item.category?.toLowerCase();
          const isWorldTrending = source === 'world' && category === 'trending';
        ...
        console.error('Error fetching latest world news for carousel:', err);
        setError('Failed to load latest world news');
        setTrendingNews([]);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="d-flex justify-content-center align-items-center py-5">
        <div className="text-center">
          ...
          <p className="text-muted mb-0">Loading latest world trending news...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || trendingNews.length === 0) {
    return (
      <div className="alert alert-info border-0 rounded-3">
        ...
        Latest world trending news carousel temporarily unavailable
      </div>
    );
  }

  return (
    <div className="latest-world-carousel mb-5">
      {/* Header */}
      <div className="d-flex align-items-center justify-content-between mb-4">
        <div className="d-flex align-items-center">
          ...
          <div>
            <h4 className="mb-0 fw-bold">Latest World Trending News</h4>
            <small className="text-muted">Stay updated with the latest trending news from around the world</small>
          </div>
        </div>
        <div className="d-flex align-items-center">
          <span className="badge bg-primary me-2">LIVE</span>
                ...
                <div className="row g-0">
                  <div className="col-md-6">
                    {getImageUrl(news) ? (
                      ...
                      >
                        <i className="bi bi-globe text-white" style={{ fontSize: '4rem' }}></i>
                      </div>
                    )}
                  </div>
                  <div className="col-md-6">
                    <div className="card-body h-100 d-flex flex-column justify-content-between p-4">
                      <div>
                        <div className="d-flex align-items-center mb-2">
                          <span className="badge bg-primary me-2">{news.category || 'World'}</span>
                        ...
                        <p className="card-text text-muted" style={{
                          ...
                          {news.description || news.content || 'Read more about this latest world news story.'}
                        </p>
                      </div>
      ...
      <div className="text-center mt-3">
        <small className="text-muted">
          Showing {currentSlide + 1} of {trendingNews.length} latest world trending articles
        </small>
      </div>
    </div>
  );
...
export default LatestWorldCarousel;
